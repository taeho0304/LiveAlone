...
public class UserFavoriteRepositorySupport {
    ...
    @Transactional
    public void deleteByIds(Long userFavoriteId, Long userId) {
        jpaQueryFactory.delete(quserFavorite).where(quserFavorite.user.id.eq(userId), quserFavorite.id.eq(userFavoriteId)).execute();
    }
}